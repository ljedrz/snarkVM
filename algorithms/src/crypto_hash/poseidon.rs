...
pub struct PoseidonSponge<F: PrimeField, const RATE: usize, const CAPACITY: usize> {
    ...
    /// The random bits already generated by `get_bits`.
...
impl<F: PrimeField, const RATE: usize> PoseidonSponge<F, RATE, 1> {
    ...
    pub fn get_bits(&mut self, num_bits: usize) -> Vec<bool> {
        if self.random_bits.len() >= num_bits {
            return self.random_bits[..num_bits].to_vec();
        }

        let bits_per_element = F::size_in_bits() - 1;
        let num_elements = (num_bits - self.random_bits.len() + bits_per_element - 1) / bits_per_element;
        ...
        for elem in src_elements.iter() {
            ...
            self.random_bits.extend_from_slice(&elem_bits[skip..]);
        }

        self.random_bits[..num_bits].to_vec()
    }
