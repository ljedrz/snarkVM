...
impl<E: PairingEngine> ToMinimalBits for CircuitVerifyingKey<E> {
    fn to_minimal_bits(&self) -> Vec<bool> {
        ...
        let mut bits = vec![];
        constraint_domain_size.to_bits_le_into(&mut bits);
        non_zero_domain_a_size.to_bits_le_into(&mut bits);
        non_zero_domain_b_size.to_bits_le_into(&mut bits);
        non_zero_domain_c_size.to_bits_le_into(&mut bits);
